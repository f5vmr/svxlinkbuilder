...
function set_locale() {
    locale="$1.UTF-8"
    echo -e "${YELLOW}Setting locale to ${locale} - ${RED} Please Wait ${YELLOW} #### ${NORMAL}" | tee -a /var/log/install.log > /dev/null
...
function which_language() {
    echo "Entering which_language function"  # debug, can remove later
    ...
    if [ -n "$locale" ]; then
        ...
        echo -e "${GREEN} #### Language set to $LANG_OPTION ($locale_short) #### Wait a moment ${NORMAL}" | tee -a /var/log/install.log > /dev/null
    fi
}
