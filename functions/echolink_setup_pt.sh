...
echo -e "$(date)" "${GREEN} Echolink esta ativo ${NORMAL}" | tee -a /var/log/install.log > /dev/null
    else
     sed -i 's/,ModuleEchoLink//' /etc/svxlink/svxlink.conf
    # removing Echolink from the MODULES= line in both Simplex and Duplex
echo -e "$(date)" "${CYAN} EchoLink esta inativo ${NORMAL}" | tee -a /var/log/install.log > /dev/null    ##nothing to do
    fi
}
