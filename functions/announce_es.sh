...
function announce {
    ...
    echo "Using logic module: $LOGIC_MODULE" >> /var/log/install.log > /dev/null 
    ...
    echo -e "${CYAN}Current CW_AMP:${WHITE} $cw_amp" >> /var/log/install.log > /dev/null 
    ...
    new_long_ident_interval=$(whiptail --title "LONG IDENT INTERVAL" --menu \
        ...
        3>&1 1>&2 2>&3)
    echo "Short Ident Interval selected: $new_short_ident_interval" >> /var/log/install.log > /dev/null 
    echo "Long Ident Interval selected: $new_long_ident_interval" >> /var/log/install.log > /dev/null

    echo -e "${YELLOW}Standby for logic changes${WHITE}" >> /var/log/install.log > /dev/null 

    #### LOGIC ID / ANNOUNCE FLAGS ####
    echo -e "${YELLOW}Reading logic enable flags for [$LOGIC_MODULE]...${WHITE}" >> /var/log/install.log > /dev/null 
    ...
    echo -e "${CYAN}Short Voice ID Enable:${WHITE} $new_short_voice_id_enable" >> /var/log/install.log > /dev/null 
    echo -e "${CYAN}Short CW ID Enable:${WHITE} $new_short_cw_id_enable" >> /var/log/install.log > /dev/null 
    echo -e "${CYAN}Short Announce Enable:${WHITE} $new_short_announce_enable" >> /var/log/install.log > /dev/null 
    echo -e "${CYAN}Long Voice ID Enable:${WHITE} $new_long_voice_id_enable" >> /var/log/install.log > /dev/null 
    echo -e "${CYAN}Long CW ID Enable:${WHITE} $new_long_cw_id_enable" >> /var/log/install.log > /dev/null 
    echo -e "${CYAN}Long Announce Enable:${WHITE} $new_long_announce_enable" >> /var/log/install.log > /dev/null 
    ...
    update_svxlink_conf() {
     ...
     #   update_key "LONG_ANNOUNCE_FILE" "$long_announce_file"
        ...
        echo -e "${GREEN}Updated identification and CW variables for [$section] in $conf${WHITE}" >> /var/log/install.log > /dev/null 
    }
