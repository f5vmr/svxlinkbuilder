...
function announce {
    ...
    new_long_ident_interval=$(whiptail --title "LONG IDENT INTERVAL" --menu \
        ...
        3>&1 1>&2 2>&3)
    echo -e "Short Ident Interval selected: $new_short_ident_interval" | sudo tee -a /var/log/install.log 
    echo -e "Long Ident Interval selected: $new_long_ident_interval" | sudo tee -a /var/log/install.log 
