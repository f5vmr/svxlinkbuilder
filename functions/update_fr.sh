...
whiptail --title "Mis à jour" --msgbox "Mis à jour les répositoires pour nodejs et l'éxpansion pou l'avenire. Tapé OK pour continuer" 8 78
	...
	echo -e "$(date)" "${YELLOW} #### updates and upgrades #### ${NORMAL}" | sudo tee -a  /var/log/install.log
...
#### BUILD ESSENTIALS ####
	...
	echo -e "$(date)" "${YELLOW} ### Installation des paquets ### ${NORMAL}" | sudo tee -a  /var/log/install.log
