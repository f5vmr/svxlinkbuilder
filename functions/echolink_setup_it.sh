...
whiptail --title "Vuoi configurare EchoLink?" --yesno "EchoLink sarà installato e configurato" 8 78
if [ $? -eq "0" ] 
then
    ## "Installing echolink"
...
echo -e "$(date)" "${GREEN} EchoLink è stato configurato ${NORMAL}" | tee -a /var/log/install.log > /dev/null
    else
     sed -i 's/,ModuleEchoLink//' /etc/svxlink/svxlink.conf
    # removing Echolink from the MODULES= line in both Simplex and Duplex
echo -e "$(date)" "${CYAN} EchoLink non è stato configurato ${NORMAL}" | tee -a /var/log/install.log > /dev/null    ##nothing to do
    fi
}
