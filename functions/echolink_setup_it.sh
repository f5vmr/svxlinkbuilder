...
echo -e "$(date)" "${GREEN} EchoLink è stato configurato ${NORMAL}" | tee -a /var/log/install.log

    else
     sed -i 's/,ModuleEchoLink//' /etc/svxlink/svxlink.conf
    # removing Echolink from the MODULES= line in both Simplex and Duplex
echo -e "$(date)" "${CYAN} EchoLink non è stato configurato ${NORMAL}" | tee -a /var/log/install.log
    ##nothing to do
    fi
}
