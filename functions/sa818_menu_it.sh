...
SA818_UPDATED=0
TITLE="Configurazione SvxLink di SA818"
...
calc_wt_size() {
    ...
    if [[ $LINES -lt 22 ]]; then
	echo "La dimensione del terminale deve essere di almeno 22 righe."
	exit 1
    fi
    if [[ $COLUMNS -lt 60 ]]; then
	echo "La dimensione del terminale deve essere di almeno 60 colonne."
	exit 1
    fi
...
request_frequency() {
	...
	ANSWER=$(whiptail				\
		    --title "$TITLE"			\
		    --inputbox "La frequenza attuale di ${RX_TX_NAME} è $CURRENT_FREQ MHz\n\nInserisci la frequenza in MHz (xxx.xxxx) :"	\
	...
	whiptail								\
	    --msgbox "La frequenza deve essere compresa tra $F_MIN e $F_MAX MHz"	\
	    $MSGBOX_HEIGHT $MSGBOX_WIDTH
    done
...
do_ctcss_select() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--scrolltext				\
		--radiolist "Seleziona il tono CTCSS di $RX_TX_NAME (usa tab per cambiare, frecce per scorrere, barra spaziatrice per selezionare)"	\
		$WT_HEIGHT $WT_WIDTH			\
		$WT_MENU_HEIGHT				\
		"${ctcss_tones[@]}"			\
		3>&1 1>&2 2>&3)
...
do_dcs_select() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--scrolltext				\
		--radiolist "Seleziona il codice DCS di $RX_TX_NAME (usa tab per cambiare, frecce per scorrere, barra spaziatrice per selezionare)"	\
		$WT_HEIGHT $WT_WIDTH			\
		$WT_MENU_HEIGHT				\
		"${dcs_codes[@]}"			\
		3>&1 1>&2 2>&3)
...
do_update_band() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--radiolist "Seleziona la banda.\n\nUsa le frecce per scorrere e la barra spaziatrice per selezionare."	\
		$WT_HEIGHT $WT_WIDTH			\
		2					\
		"VHF"	"$VHF_ON"			\
		"UHF"	"$UHF_ON"			\
...
do_update_bandwidth() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--radiolist "Seleziona la larghezza di banda (Narrow = 12,5 kHz, Wide = 25,0 kHz).\n\nUsa le frecce per scorrere e la barra spaziatrice per selezionare."	\
		$WT_HEIGHT $WT_WIDTH			\
		2					\
		"Narrow" "$NARROW_ON"			\
		"Wide"	 "$WIDE_ON"			\
...
do_update_squelch() {
	...
	ANSWER=$(whiptail				\
		    --title "$TITLE"			\
		    --inputbox "Il valore attuale dello squelch è $CURRENT_SQUELCH.\n\nInserisci un nuovo valore dello squelch (0-8) :"	\
		    $MSGBOX_HEIGHT $MSGBOX_WIDTH	\
		    "$CURRENT_SQUELCH"			\
		    3>&1 1>&2 2>&3)
	if [[ $? -ne 0 ]]; then
	...
	whiptail --msgbox "Il valore dello squelch deve essere un numero compreso tra 0 e 8" $MSGBOX_HEIGHT $MSGBOX_WIDTH
    done
...
do_update_volume() {
	...
	ANSWER=$(whiptail				\
		    --title "$TITLE"			\
		    --inputbox "Il livello del volume attuale è $CURRENT_VOLUME.\n\nInserisci un nuovo livello del volume (1-8) :"	\
		    $MSGBOX_HEIGHT $MSGBOX_WIDTH	\
		    "$CURRENT_VOLUME"			\
		    3>&1 1>&2 2>&3)
	if [[ $? -ne 0 ]]; then
	...
	whiptail --msgbox "Il volume deve essere un numero compreso tra 1 e 8" $MSGBOX_HEIGHT $MSGBOX_WIDTH
    done
...
do_update_tone() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--radiolist "Vuoi usare un tono sub-audibile?\n\nUsa le frecce per scorrere e la barra spaziatrice per selezionare."	\
		$WT_HEIGHT $WT_WIDTH			\
		3					\
		"None"	"$TONE_NONE"			\
		"CTCSS"	"$TONE_CTCSS"			\
...
do_update_port() {
	...
	ANSWER=$(whiptail				\
		    --title "$TITLE"			\
		    --inputbox "La porta seriale attualmente utilizzata per programmare il chip SA818 è ${DISPLAY_PORT}.\n\nInserisci il percorso del dispositivo seriale (ad esempio \"/dev/serial0\" o lascia vuoto per usare la porta predefinita) :"	\
		    $MSGBOX_HEIGHT $MSGBOX_WIDTH	\
		    "$CURRENT_PORT"			\
		    3>&1 1>&2 2>&3)
	if [[ $? -ne 0 ]]; then
	...
	whiptail								\
	    --msgbox "Inserisci un percorso valido per il dispositivo seriale (ad esempio \"/dev/serial0\") oppure lascia vuoto per usare la porta console predefinita di Linux"	\
	    $MSGBOX_HEIGHT $MSGBOX_WIDTH
    done
...
do_update_speed() {
    ...
    ANSWER=$(whiptail					\
		...
		--noitem				\
		--radiolist "Seleziona la velocità di connessione.\n\nUsa le frecce per scorrere e la barra spaziatrice per selezionare."	\
		$WT_HEIGHT $WT_WIDTH			\
		${#SPEEDS[@]}				\
		${speeds[@]}				\
		3>&1 1>&2 2>&3)
...
do_main_menu() {
	...
	case "$CHOICE" in
	    ...
	    "" )
		;;
	    * )	whiptail --msgbox "$CHOICE non è una scelta valida." $MSGBOX_HEIGHT $MSGBOX_WIDTH
		continue
		;;
	esac
...
usage() {
    echo "Uso: $0 [ --apply ] [ --debug ]"
    echo ""
    echo "Opzioni :"
    echo "  --apply	[Ri-]applica la configurazione salvata al SA818 (senza il menu)"
    echo "  --debug	Abilita il debug di \"sa818\""
}
...
if [[ $SA818_APPLY -ne 0 ]]; then
    do_sa818_validate
    if [[ $? -eq 0 ]]; then
	echo "$0: errore: le frequenze memorizzate sono fuori banda"
	exit 1
    fi

    do_sa818_update
    if [[ $? -eq 0 ]]; then
	echo "$0: errore: impossibile aggiornare SA818"
	exit 1
    fi

    echo "L'aggiornamento di SA818 è stato effettuato"
    exit 0
fi
