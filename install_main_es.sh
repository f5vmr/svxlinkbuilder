...
nodeoption
echo -e "$(date)" "${YELLOW} #### Node Type: $NODEOPTION #### ${NORMAL}" >> /var/log/install.log > /dev/null

echo -e "$(date)" "${YELLOW} #### Sound Card: $HID $GPIOD $card #### ${NORMAL}" >> /var/log/install.log > /dev/null	
echo -e "$(date)" "${YELLOW} #### Checking Alsa #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
#### GROUPS AND USERS ####
# clear
echo -e "$(date)" "${YELLOW} #### Creating Groups and Users #### ${NORMAL}" >> /var/log/install.log > /dev/null
source "${BASH_SOURCE%/*}/functions/groups.sh"
make_groups

#### CONFIGURATION VOICES ####
 # clear
	echo -e "$(date)" "${GREEN} #### Installing Voice Files #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
#### BACKUP CONFIGURATION ####
 # clear
	echo -e "$(date)" "${GREEN} #### Copia de seguridad de la configuración en: $CONF.bak #### ${NORMAL}">> /var/log/install.log > /dev/null
...
chmod 0440 "$SUDOERS_FILE"
 	echo -e "$(date)" "${RED} #### Descarga de archivos de configuración preparados desde los scripts #### ${NORMAL}" >> /var/log/install.log > /dev/null
    ...
    # clear
	echo -e "$(date)" "${GREEN} #### Establecer indicativo en $CALL #### ${NORMAL}" >> /var/log/install.log > /dev/null

 	sudo sed -i "s/MYCALL/$CALL/g" $CONF
 	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

	echo -e "$(date)" "${GREEN} #### Configuración del tiempo de suspensión del silenciador en 10 mS ${NORMAL}" >> /var/log/install.log > /dev/null
 	sudo sed -i s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g $CONF
 
 # clear	
	echo -e "$(date)" "${YELLOW} #### Desactivar mensajes de advertencia de distorsión de audio #### ${NORMAL}">> /var/log/install.log > /dev/null


 	sudo sed -i 's/PEAK_METER=1/PEAK_METER=0/g' $CONF

 # clear
	echo -e "$(date)" "${GREEN} #### Actualización de SplashScreen al iniciar #### ${NORMAL}" >> /var/log/install.log > /dev/null

 	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
 	sudo chmod 0775 /etc/update-motd.d/10-uname

 # clear
	echo -e "$(date)" "${YELLOW} #### Cambiar el sufijo del archivo de registro ${NORMAL}" >> /var/log/install.log > /dev/null
 ...
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Comprobación de direcciones IP #### ${NORMAL}" >> /var/log/install.log > /dev/null
	
	source "${BASH_SOURCE%/*}/functions/get_ip.sh"
	ipaddress
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Instalación del panel #### ${NORMAL}" >> /var/log/install.log > /dev/null

	source "${BASH_SOURCE%/*}/functions/dash_install_es.sh"
dash_install
 # clear
	echo -e "$(date)" "${GREEN} #### Panel instalado #### ${NORMAL}" >> /var/log/install.log > /dev/null
	...
	cd /home/pi/
	 ...
	 # clear
	echo -e "$(date)" "${GREEN} #### Configurando el nodo #### ${NORMAL}" >> /var/log/install.log > /dev/null
	...
	echo -e "$(date)" "${GREEN} #### Identification setup  #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/announce_es.sh"
	announce
	echo -e "$(date)" "${GREEN} #### Announcement setup complete  #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/tones_es.sh"
	tones
	echo -e "$(date)" "${GREEN} #### Tones setup complete  #### ${NORMAL}" >> /var/log/install.log > /dev/null	
	
	cd /home/pi
	 # clear
 	echo -e "$(date)" "${RED} #### Cambiando el móduloMetar Link #### ${NORMAL}" >> /var/log/install.log > /dev/null
source "${BASH_SOURCE%/*}/functions/modulemetar_setup_es.sh"
modulemetar
	
	 # clear
	 cd /home/pi/
	echo -e "$(date)" "${RED} #### Cambio de ModuleEchoLink  #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
    ...
    /^\[/!d
	 ...
	 # clear
	echo -e "$(date)" "${RED} #### Reinicie svxlink.service #### ${NORMAL}" >> /var/log/install.log > /dev/null	
	
 	sudo systemctl restart svxlink apache2 svxlink-node

    ##.service isn't necessary ##
echo -e "$(date)" "${GREEN} #### Instalación completa #### ${NORMAL}" >> /var/log/install.log > /dev/null
whiptail --title "Instalación completa" --msgbox "Instalación completa. Vayamos al panel " 8 78
echo -e "$(date)" "${RED} #### Complete #### ${NORMAL}" >> /var/log/install.log > /dev/null
