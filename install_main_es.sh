...
# Ensure the critical config files cannot get locked by root again
find /etc/svxlink -type f -exec chmod 664 {} \;
find /etc/svxlink -type d -exec chmod 775 {} \;
...
dash_install
 # clear
	...
	announce
	 "$(date)" "${GREEN} #### Announcement setup complete  #### ${NORMAL}" | sudo tee -a /var/log/install.log 
...
if ! grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
    echo -e "Module [$NOT_LOGIC_MODULE] no longer found in $CONF" | sudo tee -a /var/log/install.log 
