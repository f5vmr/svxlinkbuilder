...
nodeoption
echo -e "$(date)" "${YELLOW} #### Tipo di nodo: $NODEOPTION #### ${NORMAL}" | tee -a  /var/log/install.log

echo -e "$(date)" "${YELLOW} #### Scheda audio: $HID $GPIOD $card #### ${NORMAL}" | tee -a  /var/log/install.log
echo -e "$(date)" "${YELLOW} #### Verifica Alsa #### ${NORMAL}" | tee -a  /var/log/install.log
...
#### GROUPS AND USERS ####
# clear
echo -e "$(date)" "${YELLOW} #### Creazione dei gruppi e degli utenti #### ${NORMAL}" | tee -a  /var/log/install.log
...
nodeoption
echo -e "$(date)" "${YELLOW} #### Tipo di nodo: $NODE_OPTION #### ${NORMAL}" | tee -a  /var/log/install.log
...
#### CONFIGURATION VOICES ####
 # clear
	echo -e "$(date)" "${GREEN} #### Installazione dei file vocali #### ${NORMAL}" | tee -a  /var/log/install.log
...
#### BACKUP CONFIGURATION ####
 # clear
	echo -e "$(date)" "${GREEN} #### Backup della configurazione in : $CONF.bak #### ${NORMAL}"| tee -a  /var/log/install.log
...
chmod 0440 "$SUDOERS_FILE"
 	echo -e "$(date)" "${RED} #### Download dei file di configurazione preparati dagli script #### ${NORMAL}" | tee -a  /var/log/install.log
    ...
    # clear
	echo -e "$(date)" "${GREEN} #### Nominativo impostato su $CALL #### ${NORMAL}" | tee -a  /var/log/install.log

 	sudo sed -i "s/MYCALL/$CALL/g" $CONF
 	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

	echo -e "$(date)" "${GREEN} #### Impostazione dello Squelch Hangtime a 10 ms #### ${NORMAL}" | tee -a  /var/log/install.log
 	sudo sed -i s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g $CONF
 
 # clear	
	echo -e "$(date)" "${YELLOW} #### Disabilitazione dei messaggi di avviso della distorsione audio #### ${NORMAL}"| tee -a  /var/log/install.log


 	sudo sed -i 's/PEAK_METER=1/PEAK_METER=0/g' $CONF

 # clear
	echo -e "$(date)" "${GREEN} #### Aggiornamento dello SplashScreen all'avvio #### ${NORMAL}" | tee -a  /var/log/install.log

 	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
 	sudo chmod 0775 /etc/update-motd.d/10-uname

 # clear
	echo -e "$(date)" "${YELLOW} #### Modifica del suffisso del file di log #### ${NORMAL}" | tee -a  /var/log/install.log
 ...
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Verifica degli indirizzi IP #### ${NORMAL}" | tee -a  /var/log/install.log
	
	source "${BASH_SOURCE%/*}/functions/get_ip.sh"
	ipaddress
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Installazione della dashboard #### ${NORMAL}" | tee -a  /var/log/install.log

	source "${BASH_SOURCE%/*}/functions/dash_install_it.sh"
	dash_install
 # clear
	echo -e "$(date)" "${GREEN} #### Dashboard installata #### ${NORMAL}" | tee -a  /var/log/install.log
	...
	cd /home/pi/
	 ...
	 # clear
	echo -e "$(date)" "${GREEN} #### Configurazione del nodo #### ${NORMAL}" | tee -a  /var/log/install.log
	...
	echo -e "$(date)" "${GREEN} #### Configurazione dell'identificazione  #### ${NORMAL}" | tee -a  /var/log/install.log
	source "${BASH_SOURCE%/*}/functions/announce_it.sh"
	announce
	echo -e "$(date)" "${GREEN} #### Configurazione degli annunci completata  #### ${NORMAL}" | tee -a  /var/log/install.log
	source "${BASH_SOURCE%/*}/functions/tones_it.sh"
	tones
	echo -e "$(date)" "${GREEN} #### Configurazione dei toni completata  #### ${NORMAL}" | tee -a  /var/log/install.log
	cd /home/pi
	 # clear
 	echo -e "$(date)" "${RED} #### Modifica del collegamento ModuleMetar #### ${NORMAL}" | tee -a  /var/log/install.log
	...
	modulemetar
	 ...
	 # clear
	 cd /home/pi/
	echo -e "$(date)" "${RED} #### Modifica del collegamento ModuleEchoLink #### ${NORMAL}" | tee -a  /var/log/install.log
...
#	propagationmonitor
	echo "Modulo logico non utilizzato: $NOT_LOGIC_MODULE" | tee -a  /var/log/install.log
...
else echo "Il modulo [$NOT_LOGIC_MODULE] è presente in $CONF, procedo a rimuoverlo." | tee -a  /var/log/install.log
fi
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
	...
	if grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
	echo "Impossibile rimuovere il modulo [$NOT_LOGIC_MODULE] da $CONF" | tee -a  /var/log/install.log
	fi
	 # clear
	echo -e "$(date)" "${RED} #### Riavvio di svxlink.service #### ${NORMAL}" | tee -a  /var/log/install.log
	
 	sudo systemctl restart svxlink apache2 svxlink-node 

	##.service isn't necessary ##
echo -e "$(date)" "${GREEN} #### Installazione completata #### ${NORMAL}" | tee -a  /var/log/install.log
...
echo -e "$(date)" "${RED} #### Installazione completata #### ${NORMAL}" | tee -a  /var/log/install.log
