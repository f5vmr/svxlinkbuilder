...
nodeoption
echo -e "$(date)" "${YELLOW} #### Tipo di nodo: $NODE_OPTION #### ${NORMAL}" | sudo tee -a  /var/log/install.log
...
#	propagationmonitor
	echo "Modulo logico non utilizzato: $NOT_LOGIC_MODULE" | sudo tee -a  /var/log/install.log
...
else echo "Il modulo [$NOT_LOGIC_MODULE] è presente in $CONF, procedo a rimuoverlo." | sudo tee -a  /var/log/install.log
fi
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
	...
	if grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
	echo "Impossibile rimuovere il modulo [$NOT_LOGIC_MODULE] da $CONF" | sudo tee -a  /var/log/install.log
	fi
	 # clear
