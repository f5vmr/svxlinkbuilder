...
echo -e "Nessun dispositivo SA818" | sudo tee -a /var/log/install.log 
fi
#### USB SOUND CARD ####
...
echo -e "$(date)" "${YELLOW} #### Scheda audio: $HID $GPIOD $card #### ${NORMAL}" | sudo tee -a /var/log/install.log
...
#### BACKUP CONFIGURATION ####
 # clear
	echo -e "$(date)" "${GREEN} #### Backup della configurazione in : $CONF.bak #### ${NORMAL}"| sudo tee -a /var/log/install.log 
...
chmod 0440 "$SUDOERS_FILE"
 	echo -e "$(date)" "${RED} #### Download dei file di configurazione preparati dagli script #### ${NORMAL}" | sudo tee -a /var/log/install.log
    ...
    # clear
	echo -e "$(date)" "${GREEN} #### Nominativo impostato su $CALL #### ${NORMAL}" | sudo tee -a /var/log/install.log 

 	sudo sed -i "s/MYCALL/$CALL/g" $CONF
 	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

	echo -e "$(date)" "${GREEN} #### Impostazione dello Squelch Hangtime a 10 ms #### ${NORMAL}" | sudo tee -a /var/log/install.log 
 	sudo sed -i s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g $CONF
 
 # clear	
	echo -e "$(date)" "${YELLOW} #### Disabilitazione dei messaggi di avviso della distorsione audio #### ${NORMAL}"| sudo tee -a /var/log/install.log 
...
#	propagationmonitor
	echo -e "Modulo logico non utilizzato: $NOT_LOGIC_MODULE" | sudo tee -a /var/log/install.log 
...
if ! grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
    echo -e "Il modulo [$NOT_LOGIC_MODULE] non è presente in $CONF" | sudo tee -a /var/log/install.log
else echo -e "Il modulo [$NOT_LOGIC_MODULE] è presente in $CONF, procedo a rimuoverlo." | sudo tee -a /var/log/install.log
fi
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
	...
	if grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
	echo -e "Impossibile rimuovere il modulo [$NOT_LOGIC_MODULE] da $CONF" | sudo tee -a /var/log/install.log
	fi
	 # clear
	echo -e "$(date)" "${RED} #### Riavvio di svxlink.service #### ${NORMAL}" | sudo tee -a /var/log/install.log 
	
 	sudo systemctl restart svxlink apache2 svxlink-node 

	##.service isn't necessary ##
echo -e "$(date)" "${GREEN} #### Installazione completata #### ${NORMAL}" | sudo tee -a /var/log/install.log 
...
echo -e "$(date)" "${RED} #### Installazione completata #### ${NORMAL}" | sudo tee -a /var/log/install.log 
