...
echo "Nessun dispositivo SA818" | sudo tee -a  /var/log/install.log
fi
#### USB SOUND CARD ####
...
nodeoption
echo -e "$(date)" "${YELLOW} #### Tipo di nodo: $NODEOPTION #### ${NORMAL}" >> /var/log/install.log > /dev/null

echo -e "$(date)" "${YELLOW} #### Scheda audio: $HID $GPIOD $card #### ${NORMAL}" >> /var/log/install.log > /dev/null
echo -e "$(date)" "${YELLOW} #### Verifica Alsa #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
#### GROUPS AND USERS ####
# clear
echo -e "$(date)" "${YELLOW} #### Creazione dei gruppi e degli utenti #### ${NORMAL}" >> /var/log/install.log > /dev/null
source "${BASH_SOURCE%/*}/functions/groups.sh"
make_groups

#### CONFIGURATION VOICES ####
 # clear
	echo -e "$(date)" "${GREEN} #### Installazione dei file vocali #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
#### BACKUP CONFIGURATION ####
 # clear
	echo -e "$(date)" "${GREEN} #### Backup della configurazione in : $CONF.bak #### ${NORMAL}">> /var/log/install.log > /dev/null
...
chmod 0440 "$SUDOERS_FILE"
 	echo -e "$(date)" "${RED} #### Download dei file di configurazione preparati dagli script #### ${NORMAL}" >> /var/log/install.log > /dev/null
    ...
    # clear
	echo -e "$(date)" "${GREEN} #### Nominativo impostato su $CALL #### ${NORMAL}" >> /var/log/install.log > /dev/null

 	sudo sed -i "s/MYCALL/$CALL/g" $CONF
 	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

	echo -e "$(date)" "${GREEN} #### Impostazione dello Squelch Hangtime a 10 ms #### ${NORMAL}" >> /var/log/install.log > /dev/null
 	sudo sed -i s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g $CONF
 
 # clear	
	echo -e "$(date)" "${YELLOW} #### Disabilitazione dei messaggi di avviso della distorsione audio #### ${NORMAL}">> /var/log/install.log > /dev/null


 	sudo sed -i 's/PEAK_METER=1/PEAK_METER=0/g' $CONF

 # clear
	echo -e "$(date)" "${GREEN} #### Aggiornamento dello SplashScreen all'avvio #### ${NORMAL}" >> /var/log/install.log > /dev/null

 	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
 	sudo chmod 0775 /etc/update-motd.d/10-uname

 # clear
	echo -e "$(date)" "${YELLOW} #### Modifica del suffisso del file di log #### ${NORMAL}" >> /var/log/install.log > /dev/null
 ...
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Verifica degli indirizzi IP #### ${NORMAL}" >> /var/log/install.log > /dev/null
	
	source "${BASH_SOURCE%/*}/functions/get_ip.sh"
	ipaddress
 # clear
	cd /home/pi
	echo -e "$(date)" "${YELLOW} #### Installazione della dashboard #### ${NORMAL}" >> /var/log/install.log > /dev/null

	source "${BASH_SOURCE%/*}/functions/dash_install_it.sh"
	dash_install
 # clear
	echo -e "$(date)" "${GREEN} #### Dashboard installata #### ${NORMAL}" >> /var/log/install.log > /dev/null
	...
	cd /home/pi/
	 ...
	 # clear
	echo -e "$(date)" "${GREEN} #### Configurazione del nodo #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/node_setup_it.sh"
	nodeset
	#### Removal of unwanted files ####
	#echo -e "$(date)" "${YELLOW} #### Removing unwanted files #### ${NORMAL}" >> /var/log/install.log > /dev/null
	...
	echo -e "$(date)" "${GREEN} #### Configurazione dell'identificazione  #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/announce_it.sh"
	announce
	echo -e "$(date)" "${GREEN} #### Configurazione degli annunci completata  #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/tones_it.sh"
	tones
	echo -e "$(date)" "${GREEN} #### Configurazione dei toni completata  #### ${NORMAL}" >> /var/log/install.log > /dev/null
	cd /home/pi
	 # clear
 	echo -e "$(date)" "${RED} #### Modifica del collegamento ModuleMetar #### ${NORMAL}" >> /var/log/install.log > /dev/null
	...
	modulemetar
	 ...
	 # clear
	 cd /home/pi/
	echo -e "$(date)" "${RED} #### Modifica del collegamento ModuleEchoLink #### ${NORMAL}" >> /var/log/install.log > /dev/null
	source "${BASH_SOURCE%/*}/functions/echolink_setup_it.sh"
	echolinksetup
	
	 # clear
#	echo -e "$(date)" "${RED} #### Changing ModulePropagationMonitor #### ${NORMAL}" >> /var/log/install.log > /dev/null
#	source "${BASH_SOURCE%/*}/functions/propagationmonitor_setup.sh"
#	propagationmonitor
	echo "Modulo logico non utilizzato: $NOT_LOGIC_MODULE" >> /var/log/install.log > /dev/null
...
else echo "Il modulo [$NOT_LOGIC_MODULE] è presente in $CONF, procedo a rimuoverlo." >> /var/log/install.log > /dev/null
fi
...
sed -i "/^\[$NOT_LOGIC_MODULE\]/,/^\[/{
	...
	if grep -q "^\[$NOT_LOGIC_MODULE\]" "$CONF"; then
	echo "Impossibile rimuovere il modulo [$NOT_LOGIC_MODULE] da $CONF" >> /var/log/install.log > /dev/null
	fi
	 # clear
	echo -e "$(date)" "${RED} #### Riavvio di svxlink.service #### ${NORMAL}" >> /var/log/install.log > /dev/null
	
 	sudo systemctl restart svxlink apache2 svxlink-node 

	##.service isn't necessary ##
echo -e "$(date)" "${GREEN} #### Installazione completata #### ${NORMAL}" >> /var/log/install.log > /dev/null
...
echo -e "$(date)" "${RED} #### Installazione completata #### ${NORMAL}" >> /var/log/install.log > /dev/null
