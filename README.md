...
<p>There are a number of available interface boards that have a variety of uses, either as a hotspot or a repeater, or even a fill-in receiver/transceiver for an existing SVXLink repeater. The settings in this build are for either a homebrew interface board using GPIOD 23/17/8 for the Receive COS and GPIOD 24/18/7 for the PTT controller, or a fully modified CM-108 that can use 'udev' and drive the PTT and COS from the modification components.</p>
<p> There is an also intermediate version for the CM-108 where only the transmit modification has been done that will you use 'udev' for the transmit, and give you options for the receive GPIOD 23/17/8. There is also an option for your own GPIO port selection. </p> 
<p>This will be particularly useful for the users of the various hotspots, like the F8ASB or RF-Guru. The previous images for these devices are quite rigid, whereas this builder gives you the opportunity to fully integrate the up-to-date add-ons. The </p>
...
<h2>The Build</h2>
<p>This script will also install a dummy sound card for the use of a Web Socket</p> 
...
<p>Translations - Portuguese by Luis Silva CR7BUI,
