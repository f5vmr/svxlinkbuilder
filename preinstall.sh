#/bin/bash
    ...
    echo -e "${BLUE}#### Moving USB Soundcard to Position Card:0 #### ${WHITE}" | tee -a /var/log/install.log
    sudo chmod 777 /etc/modules
    sudo sh -c 'echo "blacklist snd_bcm2835" > /etc/modprobe.d/raspi-blacklist.conf'
    echo -e "${BLUE}#### Blacklisting snd_bcm 2835 #### ${WHITE}" | tee -a /var/log/install.log
    sudo sed -i 's/options snd-usb/\#options snd-usb/g' /lib/modprobe.d/aliases.conf
    echo -e "${BLUE}#### Disabling HMDI Sound - Not required for this build #### ${WHITE}"| tee -a /var/log/install.log
    ...
    echo -e "${BLUE}#### Setting up Loopback Soundcard #### ${WHITE}" | tee -a /var/log/install.log
    echo snd-aloop > /etc/modules
    sudo chmod 775 /etc/modules
    sudo cp -f /home/pi/svxlinkbuilder/configs/loopback.conf /etc/asound.conf
    echo -e "${BLUE}#### Reseting alsa-state.service #### ${WHITE}" | tee -a /var/log/install.log
    ...
    echo -e "${BLUE}#### Adding svxlink user and group #### ${WHITE}" | tee -a /var/log/install.log
    sudo useradd -rG audio,plugdev,gpio,dialout svxlink
    sudo apt update && sudo apt upgrade -y
   
    sudo wget -O /var/cache/apt/archives/svxlink-25.5.2.3.g7bdf09d2-Linux.deb \
    https://github.com/f5vmr/svxlink/releases/download/V25.5.2/svxlink-25.5.2.3.g7bdf09d2-Linux.deb
    sudo apt install /var/cache/apt/archives/svxlink-svxlink-25.5.2.3.g7bdf09d2-Linux.deb -y

    echo -e "${BLUE}#### Installing svxlink #### ${WHITE}" | tee -a /var/log/install.log
    sudo apt install -y curl apache2 apache2-bin apache2-data apache2-utils php8.2 python3-serial nodejs npm js toilet --fix-missing -y
    ...
    echo -e "${RED} #### svxlink.service enabled for next reboot #### ${NORMAL}" | tee -a  /var/log/install.log 
    ## installing locales.##
    ## Set en_GB.UTF-8 as the default locale to begin with.##
    sudo localectl set-locale LANG=en_GB.UTF-8
    echo -e "${CYAN}Locale setup completed.${WHITE}"
    echo -e "${YELLOW}#### Your next command / prochain commande / próximo comando will be ${GREEN}./svxlinkbuilder/install.sh #### ${WHITE}" | tee -a /var/log/install.log
    echo -e "${GREEN}#### Login after Rebooting  then ${BLUE} ./svxlinkbuilder/install.sh #### ${WHITE}" | tee -a /var/log/install.log
    sudo shutdown -r now