#/bin/bash
    ...
    echo -e "${BLUE}#### Moving USB Soundcard to Position Card:0 #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    sudo chmod 777 /etc/modules
    sudo sh -c 'echo "blacklist snd_bcm2835" > /etc/modprobe.d/raspi-blacklist.conf'
    echo -e "${BLUE}#### Blacklisting snd_bcm 2835 #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    sudo sed -i 's/options snd-usb/\#options snd-usb/g' /lib/modprobe.d/aliases.conf
    echo -e "${BLUE}#### Disabling HMDI Sound - Not required for this build #### ${WHITE}"| sudo tee -a /var/log/install.log > /dev/null    
    ...
    echo -e "${BLUE}#### Setting up Loopback Soundcard #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    echo snd-aloop > /etc/modules
    sudo chmod 775 /etc/modules
    sudo cp -f /home/pi/svxlinkbuilder/configs/loopback.conf /etc/asound.conf
    echo -e "${BLUE}#### Reseting alsa-state.service #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    ...
    echo -e "${BLUE}#### Adding svxlink user and group #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    ...
    echo -e "${BLUE}#### Installing svxlink #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    ...
    echo -e "${BLUE}#### Installing locales #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    # Must enable the service.service to avoid a problem later ##
    
    sudo systemctl enable svxlink.service
    echo -e "${RED} #### svxlink.service enabled for next reboot #### ${NORMAL}" | sudo tee -a /var/log/install.log > /dev/null 
    ## installing locales.##
    ## Set en_GB.UTF-8 as the default locale to begin with.##
    sudo localectl set-locale LANG=en_GB.UTF-8
    echo -e "${CYAN}Locale setup completed.${WHITE}"
    echo -e "${YELLOW}#### Your next command / prochain commande / próximo comando will be ${GREEN}./svxlinkbuilder/install.sh #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    echo -e "${GREEN}#### Login after Rebooting  then ${BLUE} ./svxlinkbuilder/install.sh #### ${WHITE}" | sudo tee -a /var/log/install.log > /dev/null    
    sudo shutdown -r now